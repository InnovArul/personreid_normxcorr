        ...
        if file ~= "." and file ~= ".." then
            ...
            if attr.mode == "directory" then
                ...
                for index = 1, 5 do
                    ...
                    imgSavePath = paths.concat(sourcepath, 'trans-flip-' .. filename .. '-' .. index  .. extension);
                    image.save(imgSavePath, imgFlipped)
                    print('saved image with translated flip')                    
                end
                
                -- save the flipped image
                imgFlipped = image.hflip(img)
                imgSavePath = paths.concat(sourcepath, 'just-flip-' .. file);
                image.save(imgSavePath, imgFlipped)
